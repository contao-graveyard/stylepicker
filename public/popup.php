...
class Stylepicker4ward_Wizard extends Backend
{
    ...
    public function compile(): void
    {
        ...
        if (!preg_match('~^[a-z0-9_\\-]+$~i', (string) $tbl)) {
            exit('unexpected chars in tbl-param');
        }
        if (!preg_match('~^[a-z0-9_\\-]*$~i', (string) $sec)) {
            exit('unexpected chars in sec-param');
        }
    ...
    public function run(): void
    {
        ...
        if (!strlen((string) $this->Template->headline)) {
        ...
        $this->Template->request = StringUtil::ampersand($this->Environment->request);
